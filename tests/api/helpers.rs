...
use personal_ledger_server::{configuration::Configuration, startup::Application, telemetry};
...
pub async fn spawn_app(database_pool: Pool<Postgres>) -> Result<TestApp> {
    ...
    let application = Application::build(configuration.clone(), database_pool.clone())
        .await
